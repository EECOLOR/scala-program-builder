...
trait Brancher[F[_], G[_], L] {
  type Out[x]
  def branch[R, A](right: F[A])(branch: A => Either[G[L], R]): Program[Out]#Instance[R]
}
...
object Brancher {
  ...
  implicit def forProgram[F[_], G[_], L, O[_]](
    ...
    new Brancher[F, G, L] {
      ...
      def branch[R, A](right: F[A])(branch: A => Either[G[L], R]): Program[O]#Instance[R] = {
        import Implicits._
        right.flatMap { a =>
          branch(a) match {
            case Left(left) => left flatMap Branch[L, R]
            case Right(r)   => Program(r)
          }
        }
    }
  }
